...
| **Team lab (trusted peers)** | One daemon + one token for a cooperating team is a supported ops pattern ([Remote sandbox host](https://grainvm.com/guides/remote-host/)). Everyone with the token is equivalent. |

Related risks already documented elsewhere: **overlay** shared L2 (guest agents reachable by peers), **MCP** (control-plane power over Streamable HTTP — keep on loopback + token), **egress proxy** (default bind for SLIRP; firewall on multi-user hosts). Details: [Security model](https://grainvm.com/explain/security/).
