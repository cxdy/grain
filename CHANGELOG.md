...
- **Screenshot paste over `grain sh` (Cmd/Ctrl+V)** — shell WebSocket read limit raised above the 32KiB default so base64 clipboard replies (real screenshots) are not rejected; guest tools that read the host clipboard on paste no longer fail with empty/timeout errors on large images.
