...
Supported hosts: **Linux or macOS** only (not Windows/WSL). Prefer **Linux with KVM**.
...
- [Configuration]({{ '/reference/config/' | relative_url }}) — `api`, `api_url`, `api_token`, caps  
- [Security model]({{ '/explain/security/' | relative_url }})  
