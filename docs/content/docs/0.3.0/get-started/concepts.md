...
- SDKs: [Go](/docs/0.3.0/reference/go-sdk/) · [TypeScript](/docs/0.3.0/reference/typescript-sdk/) · [Python](/docs/0.3.0/reference/python-sdk/)  


Optional Bearer auth: set `api_token` in config and `GRAIN_TOKEN` in the environment.

For a **shared remote machine** (daemon as a service, teammates over SSH or SDKs), see [Remote sandbox host](/docs/0.3.0/guides/remote-host/). The CLI uses the **local unix socket** on the machine where it runs; remote automation uses the TCP API or an SSH session on the host.
