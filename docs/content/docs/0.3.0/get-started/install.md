...
**Windows (native):** not supported as a grain host. From Windows, drive a remote grain daemon on macOS or Linux via the [HTTP API](/docs/0.3.0/reference/api/) or [SDKs](/docs/0.3.0/reference/go-sdk/).
...
After install, the script asks whether to **enable MCP by default** (`mcp.enabled` in `~/.grain/config.yaml`). Non-interactive installs skip the prompt; use `GRAIN_ENABLE_MCP=1` or `0` to force yes/no. If you skip, you can enable later with `grain up --mcp` — see [MCP server](/docs/0.3.0/guides/mcp/).
...
- [Quick start](/docs/0.3.0/get-started/quickstart/) — starter config + first sandbox in one page  
- [Your first sandbox](/docs/0.3.0/get-started/first-sandbox/) — guided tutorial + interactive demo
