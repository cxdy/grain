...
| **Readiness** | `GET /readiness` · fields on `/health` | `grain status [name]` · bootstrap wait — see [Readiness protocol](/docs/0.4.0/explain/readiness/) |
...
| `--wait bootstrap` | agent healthy, then readiness protocol `state=ready` (see [Readiness protocol](/docs/0.4.0/explain/readiness/)) |

Golden images (`grain-ubuntu` via `grain image import` / bake) set `has_agent` so create prefers probing the agent before SSH deploy. See [images.md](/docs/0.4.0/guides/images/).
...
- [images.md](/docs/0.4.0/guides/images/) — golden agent-baked images  
- [recipes/coding-agent.md](/docs/0.4.0/guides/recipes/coding-agent/) — mount repo, run agent, cp results  
- [recipes/ci-ephemeral.md](/docs/0.4.0/guides/recipes/ci-ephemeral/) — create → x → rm  
- [troubleshooting.md](/docs/0.4.0/guides/troubleshooting/) — doctor, logs, timeouts  
