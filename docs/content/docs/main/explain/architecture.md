...
Names and roles without the full system diagram.
{{< /only-need >}}
...
The daemon is the source of truth for which VMs exist, their ports, and disks. The CLI is a thin client. Prefer the HTTP API or SDKs for automation so behavior matches across tools.
...
SSH works well for interactive login and bootstrap. The agent is better for:
