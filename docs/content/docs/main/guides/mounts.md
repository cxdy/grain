...
Or a `#cloud-config` document with `runcmd` / `mounts` keys—grain merges list keys (`runcmd`, `packages`, …) with its base config. See [Images](/docs/0.2.2/guides/images/) for the default cloud image and SSH user.

## Related

- [Networking](/docs/0.2.2/guides/networking/) — SLIRP and `--publish`
- [Troubleshooting](/docs/0.2.2/guides/troubleshooting/) — cloud-init and serial logs
